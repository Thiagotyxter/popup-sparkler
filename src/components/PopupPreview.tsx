            ...
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-wide">Clique pra copiar</span>
                </div>
                <div
                  ...
                  <SelectContent>
                    {state.quantityOptions.map((option) => (
                      <SelectItem key={option.id} value={option.id}>
                        {option.quantity} UNIDADE{option.quantity > 1 ? "S" : ""}{" "}
        ...
        <div className="px-8 pb-6">
          <div className="bg-emerald-50 rounded-2xl p-6 border-2 border-emerald-200 shadow-sm">
            <div className="text-center space-y-2">
              <div className="text-5xl font-extrabold text-emerald-950">{getCurrentPrice()}</div>
              ...
              {state.enableSubscription && purchaseType === "one-time" && selectedQuantity && (
                <p className="text-sm font-medium text-emerald-700">
                  ...
                  {state.quantityOptions.find((o) => o.id === selectedQuantity)?.quantity > 1 ? "s" : ""}
                </p>
              )}
              {(!state.enableSubscription || (!selectedQuantity && !selectedPlan)) && state.priceOriginal && (
                <div className="text-base text-muted-light line-through opacity-60">{state.priceOriginal}</div>
              )}
            </div>
          </div>
        </div>
